...
## 0 you should do:
* 4.1 中一定要修改voc_root
* 4.2 中不需要修改（如果你本来不是pascal voc，则需要手动增加.names文件）
* 在weights文件夹中添加权重文件```yolov3spp-voc-512.pt```
* 使用train.py进行训练
* 训练完毕后会生成```./run/Oct28_17-55-29_wz/events.out.tfevents.1603791769.localhost.localdomain.178338.0```文件。进入到```./run/Oct28_17-55-29_wz```文件夹后，在terminal中使用```tensorboard.exe --logdir "./"```。进入显示的端口号就可以看到评价指标。
* 在```predict_test.py```文件中，修改```weight=""```为生成的自己的权重文件。将需要测试的图片放在根目录下```./test.jpg```
## 1 环境配置：
* Python3.6或者3.7
